...
<input name="csrf_token" type="hidden" value="<?= app()->auth::generateCSRF() ?>"/>
       ...
       <select name="position_id" class="flex justify-center gap-4 bg-neutral-300 text-black m-4 p-2 rounded-full w-96 text-lg">
            <option value="1">врач-хирург</option>
            <option value="2" >врач-невролог</option>
            <option value="3">врач-педиатр</option>
            <option value="4">врач-психиатр</option>
        </select>
        <select name="specialization_id" class="flex justify-center gap-4 bg-neutral-300 text-black m-4 p-2 rounded-full w-96 text-lg">
